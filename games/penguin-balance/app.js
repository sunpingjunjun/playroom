...
window.addEventListener('pointermove',track);window.addEventListener('pointerup',e=>{if(!drag||e.pointerId!==drag.id)return;const d=drag,t=targetAt(e.clientX,e.clientY);cancelDrag();if(d.moved){if(t)move(d.item,t)}else{selection=d.item;d.button.classList.add('picked');$('hint').textContent='おさらを えらぼう'}});window.addEventListener('pointercancel',cancelDrag);window.addEventListener('blur',cancelDrag);window.addEventListener('keydown',e=>{if(e.key==='Escape'){cancelDrag();clearPick();$('hint').textContent='つかんで おさらに のせよう'}});
